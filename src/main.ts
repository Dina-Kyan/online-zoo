...
async function loadPets() {
  ...
  try {
    ...
    document.getElementById("pets-right")?.addEventListener("click", () => swapPets("next", container, pets));
    document.getElementById("pets-left")?.addEventListener("click", () => swapPets("prev", container, pets));
    ...
    if (diff > 50) swapPets("prev", container, pets);   
    if (diff < -50) swapPets("next", container, pets);  
});
...
function renderPets (container: HTMLElement, pets: Pet[]) {
  container.innerHTML = "";
...
function swapPets(direction: "next" | "prev", container: HTMLElement, pets: Pet[]) {
  if (direction === "next") {
...
async function loadFeedback() {
  ...
  try {
    ...
    document.getElementById("right-feedback")?.addEventListener("click", () => swapFeedback("next", container, feedbacks));
    document.getElementById("left-feedback")?.addEventListener("click", () => swapFeedback("prev", container, feedbacks));
    ...
    if (diff > 50) swapFeedback("prev", container, feedbacks);   
    if (diff < -50) swapFeedback("next", container, feedbacks);  
    });
...
function renderFeedbacks (container: HTMLElement, feedbacks: Feedback[]) {
  container.innerHTML = "";
...
function swapFeedback(direction: "next" | "prev", container: HTMLElement, feedbacks: Feedback[]) {
  if (direction === "next") {
